...
function createJob(req, res) {
  ...
  req.on('end', () => {
    ...
    if (errors.length > 0) {
      ...
      .then(async () => {
        // move original file
        let originalLocation = bodyObj.pathname;
        let newLocation = path.join(outputDir, path.basename(originalLocation));
        await fs.copyFile(originalLocation, newLocation);
        await fs.rm(originalLocation);
        return newLocation;
      })
      .then((newOriginal) => {
...
function simpleReturn(req, res, status, message) {
  ...
  switch(status) {
    ...
    case '405': 
      ...
      res.end();
      debug('[simpleReturn] 405 case complete')
      break;

    default:
      debug('[simpleReturn] default with status', status);
      ...
      debug('[simpleReturn] ending with message', message);
      message && res.write(message);
      res.end();
      debug('[simpleReturn] default case complete')
      break;
  }
}
